...
class Affine_transfer:
    ...
    def transform(self, image):
        ...
        if short_side == 0:
            # 上辺をtrans_raito倍にする
            dst_points = [(cut_width, 0), (image.shape[1] - cut_width, 0), (image.shape[1], image.shape[0]), (0, image.shape[0])]
        elif short_side == 1:
            # 右辺をtrans_raito倍にする
            dst_points = [(0, 0), (image.shape[1], cut_height), (image.shape[1], image.shape[0] - cut_height), (0, image.shape[0])]
        elif short_side == 2:
            # 下辺をtrans_raito倍にする
            dst_points = [(0, 0), (image.shape[1], 0), (image.shape[1] - cut_width, image.shape[0]), (cut_width, image.shape[0])]
        elif short_side == 3:
            # 左辺をtrans_raito倍にする
            dst_points = [(0, cut_height), (image.shape[1], 0), (image.shape[1], image.shape[0]), (0, image.shape[0] - cut_height)]
        print(dst_points)
